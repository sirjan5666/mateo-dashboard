...
import { Activity, Milk, Baby as BabyIcon, Syringe, Thermometer } from 'lucide-react';
...
const LEVEL_COLOR: Record<'ok' | 'watch' | 'urgent', string> = {
  ok: '#0f9d6e',
  watch: '#d97706',
  urgent: '#e11d48',
};
    ...
    <Card className="p-5">
      ...
      {snap.allergies.length > 0 && (
        <div className="mt-3 rounded-xl border border-rose-200 bg-rose-50 p-3">
          <p className="text-xs font-bold uppercase tracking-wide text-rose-700">⚠ Allergies</p>
          <p className="mt-0.5 text-sm text-rose-800">
            {snap.allergies.map((a) => `${a.name} (${a.severity})${a.reaction ? ` — ${a.reaction}` : ''}`).join(', ')}
          </p>
        </div>
      )}
      ...
      <div className="mt-3 grid grid-cols-2 gap-2 sm:grid-cols-4">
        ...
        <Stat icon={Milk} label="Feeds today" value={String(snap.feeds.feedsToday)} sub={snap.feeds.breastMinToday > 0 ? `${snap.feeds.breastMinToday}m nursing` : undefined} />
        <Stat icon={BabyIcon} label="Nappies today" value={`${snap.diapers.wetToday}W / ${snap.diapers.dirtyToday}D`} />
      </div>
      ...
      <div className="mt-3 grid grid-cols-1 gap-3 sm:grid-cols-2">
        ...
        <div className="rounded-xl border border-stone-100 p-3">
          <p className="flex items-center gap-1.5 text-xs font-semibold uppercase tracking-wide text-stone-400">
            <Thermometer className="h-3.5 w-3.5" /> Recent fever / symptoms
          </p>
          {snap.symptoms.length === 0 ? (
            <p className="mt-1 text-sm text-stone-400">None logged.</p>
          ) : (
            <ul className="mt-1 space-y-1">
              {snap.symptoms.map((s, i) => (
                <li key={i} className="flex items-center justify-between text-sm">
                  <span className="text-stone-600">{formatDateIST(s.loggedAt)}</span>
                  <span className="font-semibold" style={{ color: LEVEL_COLOR[s.level] }}>
                    {typeof s.temperatureC === 'number' ? `${s.temperatureC}°C` : '—'}
                    {s.level !== 'ok' && <span className={cn('ml-1 text-xs font-normal capitalize')}>· {s.level}</span>}
                  </span>
                </li>
              ))}
            </ul>
