...
function EmptyState() {
  return (
    <Card className="px-6 py-10 text-center">
      ...
      <h2 className="mt-3.5 font-display text-xl font-semibold text-stone-900">No appointments yet</h2>
      <p className="mx-auto mt-1 max-w-xs text-sm leading-relaxed text-stone-500">
        Once parents book a consultation with you, it will show up here — grouped and ready to attend.
      </p>
    </Card>
  );
}
...
export default function DoctorAppointments() {
  ...
  const cancelBtn = (c: Consultation) => (
    <button onClick={() => void setStatus(c.id, 'cancelled')} disabled={busyId === c.id} className="px-2 text-sm font-semibold text-rose-600 hover:text-rose-700 disabled:opacity-50">Cancel</button>
  );
    ...
    <>
      ...
      {c.status === 'booked' && (
        <div className="ml-auto flex items-center gap-2">
          {cancelBtn(c)}
          {c.meetLink && <a href={c.meetLink} target="_blank" rel="noopener noreferrer" className={buttonClass('secondary', 'sm')}><Video className="h-3.5 w-3.5" />Join</a>}
          <button onClick={() => void setStatus(c.id, 'completed')} disabled={busyId === c.id} className={buttonClass('primary', 'sm')}><Check className="h-3.5 w-3.5" />Mark completed</button>
        </div>
      )}
    </>
  );
  ...
  const heroActions = (c: Consultation) => (
    <>
      {cancelBtn(c)}
      <Link to={`/doctor/consultations/${c.id}`} className={buttonClass('secondary', 'md')}><MessageCircle className="h-4 w-4" />Open chat</Link>
      {c.meetLink && <a href={c.meetLink} target="_blank" rel="noopener noreferrer" className={buttonClass('secondary', 'md')}><Video className="h-4 w-4" />Join</a>}
      <button onClick={() => void setStatus(c.id, 'completed')} disabled={busyId === c.id} className={buttonClass('primary', 'md', 'shadow-card')}><Check className="h-4 w-4" />Mark completed</button>
    </>
  );
    ...
    <div className="mx-auto max-w-3xl">
      <header className="flex items-center gap-3">
        ...
        <div>
          ...
          <h1 className="font-display text-2xl font-extrabold leading-tight text-stone-900">Appointments</h1>
          <p className="text-sm text-stone-500">Consultations booked with you.</p>
        </div>
      </header>
      ...
      {items === null ? (
        ...
        <div className="mt-5 flex flex-col gap-5">
          ...
          {tab === 'upcoming' &&
            (counts.upcoming === 0 ? (
              <ConsultationsMiniEmpty msg="No upcoming appointments right now." />
            ) : (
          ...
          {tab === 'completed' &&
            (counts.completed === 0 ? (
              <ConsultationsMiniEmpty msg="No completed appointments yet." />
            ) : (
          ...
          {tab === 'cancelled' &&
            (counts.cancelled === 0 ? (
              <ConsultationsMiniEmpty msg="No cancelled appointments." />
            ) : (
